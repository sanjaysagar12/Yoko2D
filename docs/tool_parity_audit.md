...
| `EndLine` | `tools/drawTools/toolpoint/toolsinglepoint/toollinepoint/vtoolendline.cpp` | `ToolKind::EndLine` | Implemented-Correct | Source confirmed: `line.setAngle(angle); line.setLength(length);`. Verified by the exact same Qt `QLineF::setAngle`/`angle()` convention analysis done for `Normal` below (see that row/test) — Qt's angle-based line construction maps directly onto Yoko2D's plain `base + length*(cos,sin)` formula with no sign difference. |
